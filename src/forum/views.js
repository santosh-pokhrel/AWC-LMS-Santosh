  ...
  <div current-post-id="{{: postId }}" class="relative bg-white rounded p-3 mb-4 postCard border border-[#F2F2F2]">
    ...
    <div class="flex items-center gap-4">
      <img class="w-6 h-6 rounded-full border border-[#d3d3d3]"
           src="https://i.ontraport.com/265848.fef288b2e2a570a362d1141a783309d8.JPEG">
      <div class="flex items-center gap-4">
    ...
    <!-- COMMENTS -->
    <div class="mt-4" id="comments-{{:postId}}">
      {{if Comment && Comment.length}}
        <div class="text-center text-[#586a80] text-sm my-2">
          {{:Comment.length}} Comments
        </div>

        {{for Comment}}
          <div class="flex gap-3 py-4 border-l-2 pl-4">
            <img class="w-6 h-6 rounded-full border border-[#d3d3d3]"
                 src="{{:avatar || 'https://i.ontraport.com/265848.fef288b2e2a570a362d1141a783309d8.JPEG'}}">
            <div class="flex-1">
              <div class="flex items-center gap-3">
                <div class="text-[#414042] button">{{:#data.author}}</div>
                {{if designation}}
                  <div class="py-1 px-2 bg-[#c7e6e6] rounded-[36px] text-[12px] text-[#414042]">{{:designation}}</div>
                {{/if}}
                <div class="w-[2px] h-[15px] bg-[#bbbcbb] max-[600px]:hidden"></div>
                <div class="text-[#586a80] text-[12px]">{{:published}}</div>
              </div>

              <div class="mt-2 text-[#414042]">
                {{:#data.comment}}
              </div>

              <div class="mt-2 flex items-center gap-4">
                <button class="roundedButton" data-action="upvote-comment" data-comment-id="{{:id}}">
                ...
                <div class="text-[#007b8e] text-label cursor-pointer"
                     data-action="open-reply" data-comment-id="{{:id}}">Reply</div>
              </div>
            </div>
          </div>
        {{/for}}
    ...
    <div class="mt-4 commentForm hidden" id="commentForm_{{:postId}}"
         data-parent-id="{{:postId}}" data-parent-type="post" data-forum-post-id="{{:postId}}">
  ...
  toggleCreateForumSection(element){
   ...
   getCommentValueObj(){
    ...
    document.addEventListener("submit", async (e)=>{
     ...
     let result = await this.model.createComment({html: html, forumId: Number(parentId)})
      if (!result.isCancelling){
        alert("New comment has been created")
      }else{
        alert("Comment creatio failed")
      }
